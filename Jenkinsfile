pipeline {
    ...
    triggers {
        cron('H 2 * * *') // Executa todos os dias por volta das 2h
    }
    ...
    post {
        success {
            mail to: "${EMAIL}",
                 ...
                 body: "O job de integração e entrega contínua da API ${IMAGE_NAME} foi executado com sucesso."
        }
        failure {
            mail to: "${EMAIL}",
                 subject: "Falha no Deploy - ${IMAGE_NAME}",
                 body: "O job de integração e entrega contínua da API ${IMAGE_NAME} falhou. Verifique os logs no Jenkins."
        }
    }
}
